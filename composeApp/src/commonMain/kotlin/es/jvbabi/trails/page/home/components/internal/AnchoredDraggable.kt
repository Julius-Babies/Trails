package es.jvbabi.trails.page.home.components.internal
...
internal class AnchoredDraggableState<T>(
    ...
    val animationSpec: (T) -> AnimationSpec<Float>,
    ...
    constructor(
        ...
        animationSpec: (T) -> AnimationSpec<Float>,
    ...
    private fun computeTarget(offset: Float, velocity: Float): T {
        val currentAnchors = anchors
        val isMoving = abs(velocity) > 0.0f
        val isMovingForward = isMoving && velocity > 0f
        return if (!isMoving) {
            currentAnchors.closestAnchor(offset)!!
        } else if (abs(velocity) >= abs(velocityThreshold())) {
            currentAnchors.closestAnchor(offset, searchUpwards = isMovingForward)!!
        } else {
            val left = currentAnchors.closestAnchor(offset, false)!!
            val leftAnchorPosition = currentAnchors.positionOf(left)
            val right = currentAnchors.closestAnchor(offset, true)!!
            ...
            val relativePosition = abs(closestAnchorFromStart - offset)
...
internal suspend fun <T> AnchoredDraggableState<T>.animateTo(
    ...
    anchoredDrag(targetValue = targetValue) { anchors, latestTarget ->
        ...
        if (!targetOffset.isNaN()) {
            ...
            animate(prev, targetOffset, velocity, animationSpec(latestTarget)) { value, velocity ->
